...
AttributeEditor::AttributeEditor(LAttribute* attr, QWidget* parent) :
	m_attr{ attr },
	m_links_view{ new LinksView(attr) },
	QLWidget(parent)
{
	init_attributes();
	init_layout();
	...
	VTab* links_tab = m_features_tab_bar->tabs().last();
	...
	connect(m_new_link_button, &QLButton::clicked,
		[this]
		{
			if (m_status_states->state() == "Inactive")
			{
				...
				//m_new_link_widget = new NewLinkWidget(m_attr);
				m_new_link_widget->apply_definition(definition()->find_item(
						"Links Widget/New Link Widget"));

				m_links_layout->insertWidget(1, m_new_link_widget);

				m_new_link_widget_destroyed_connection =
					connect(m_new_link_widget, &NewLinkWidget::destroyed,
	...
	if (attr)
	{
		...
		if (attr->type_index() == 3 || attr->type_index() == 4)
		{
			...
			m_line_editor->hide();
			m_slider->hide();
			m_link_displayer->hide();
		}
		else if (attr->type_index() == 1)
		{
			m_line_editor->set_text(QString::number(attr->as<double>()));
			m_line_editor->text()->create_link(attr);

			m_slider->value()->create_link(m_line_editor->text());
		...
		if (attr->link())
		{
			...
			m_link_displayer->set_link(attr->link());
		}
		else
		{
			m_break_link_button->hide();
		...
		if (attr->parent())
			if (LDefinition* parent_theme_item = dynamic_cast<LDefinition*>(attr->parent()))
		...
		if (attr->has_states())
		{
			for (const auto& [key, state_attr] : attr->states())
			{
				AttributeEditor* state_editor =
					new AttributeEditor(state_attr);

				state_editor->set_object_name("Attribute Editors");
				add_share_definable(state_editor);

				m_overrides_layout->addWidget(state_editor);
			}
		}
	}
	else
...
QList<QLDefinable*> AttributeEditor::child_qldefinables(
	Qt::FindChildOptions options)
{
	QList<QLDefinable*> child_qldefinables =
		QLDefinable::child_qldefinables(options);

	child_qldefinables.append(m_features_tab_bar);
	child_qldefinables.append(m_links_widget);
	child_qldefinables.append(m_overrides_widget);

	return child_qldefinables;
}

FFillControl* AttributeEditor::fill_control() const
{
	return m_fill_control;
}

QLLineEditor* AttributeEditor::line_editor() const
{
	return m_line_editor;
}

QLMiniSlider* AttributeEditor::slider() const
{
	return m_slider;
}

void AttributeEditor::update_icon_labels()
{
	if (m_attr->link())
